...
Why it matters: a finished task is not a safe run. Detection is not containment. A model ranking that ignores harness phase will rank the wrong thing.
...
Why it matters: skills are becoming executable programs. If the model chooses the access mode, the sandbox budget is just a hope. The control plane should own physical execution shape.
...
Harnesses and skills are execution substrates. Measure them by phase, not by completed tasks, and do not let generated code choose its own memory shape.
